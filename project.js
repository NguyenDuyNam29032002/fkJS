...
const getBet = (blance, lines) => {
  ...
  if (isNaN(numberBet) || numberBet <= 0 || numberBet > blance / lines) {
...
const spin = () => {
  ...
  for (let index = 0; index < COLS; index++) {
    ...
    for (let j = 0; j < ROWS; j++) {
      ...
      reels[index].push(selectedSymbol);
...
const game = () => {
  ...
  while (true) {
    console.log("u have a balance of $".blance);
    ...
    if (balance <= 0) {
      console.log("u ran out $");
      break;
    }

    const playAgain = promt("do u want play again y/n: ");
